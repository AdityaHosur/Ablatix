...
import { ShieldCheck, ArrowRight, Lock, Mail, Building2, Radio } from "lucide-react";
...
const handleAuth = async (e: React.FormEvent) => {
  ...
  try {
    ...
    // 🔥 ensure minimum 2 seconds loader
...
return ( <main className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-[family-name:var(--font-geist-sans)] text-slate-900"> <div className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/60 p-10 border border-slate-100 animate-in fade-in zoom-in duration-300">
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input
                required
                type="text"
                placeholder="Brand Marketing team"
                value={companyName}
                onChange={(e) => setCompanyName(e.target.value)}
                className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-slate-50 border border-slate-100 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-medium text-slate-700"
              />
            </div>
          </div>
